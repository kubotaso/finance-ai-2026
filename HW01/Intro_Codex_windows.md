...
> - ChatGPT（Codex）アプリの代わりに、Windows版Claudeデスクトップアプリをインストールして使用します。
...
1. [ChatGPTデスクトップアプリ](https://learn.chatgpt.com/docs/windows/windows-app)をWindowsにインストールします。Microsoft Storeを使ってください。
2. アプリを起動し、ChatGPTアカウントでサインインします。
3. アプリの **Codex** を選びます。
4. 新しいチャットを開き、実行環境で **Local** を選びます。
5. エクスプローラーでドキュメント（Documents）フォルダを開き、`金融論`フォルダ、その中に`HW01`フォルダを作ります。
6. Codexで`ドキュメント\金融論\HW01`フォルダを開きます。
...
- `Enter`／`Return`：改行
- `Ctrl + Enter`：入力した指示を送信

Codex appの設定で、`Enter`／`Return`だけでは送信せず、`Ctrl + Enter`で送信するように変更します。

> 補足：Codexをより本格的な開発に使う場合は、[WSL2](https://learn.chatgpt.com/docs/windows/wsl)を利用する方法も適しています。WSL2はWindows上でLinux環境を使う仕組みです。Linux用のソフトを使う場合や、開発するファイルをWSL2内に置く場合に向いています。この宿題では設定する必要はありません。
...
- **Model（モデル）**：指示を読んで考え、回答やファイルを作るAIの種類です。
  ...
  - **Terra**：日常的な作業を、性能と速さのバランスを取りながら進めるモデルです。
...
>
> 動画収録時に説明したSol、Terra、Lunaに加えて、現在はAstraが表示される場合があります。実際の作業では、画面に表示されているモデルから選びます。
...
- **winget**：Windowsにソフトをインストールするための機能です。
- **Python**：データを処理するためのプログラミング言語です。Web上の公開データを取得し、必要な期間や項目を選び、合計や増加率を計算して、グラフを作るときに使います。この宿題では、第10章のデータとグラフを最新のものへ更新するために使います。
- **Node.js**：JavaScriptというプログラミング言語で作られた道具を、Windows上で動かすための実行環境です。この宿題では、Marpの変換機能を動かし、Markdownで書いたスライドをPDFへ変換するために使います。自分でJavaScriptを書く必要はありません。
...
> **🚨 動画での修正点**
>
> - wingetは、windowsに最初から入っているようです。
> - しかし、権限の問題からCodexではそのまま使えません。
> - <span class="red-text">Codexをフルアクセスに指定してください。</span>
> - VS Codeのインストールが終わったら、権限を元に戻してください。


wingetを使うと、Codexから必要なソフトをまとめてインストールできます。命令を暗記する必要はありません。

まず、Codexに次のように指示します。

```text
このWindows PCでwingetが使えるか確認して。winget --versionを実行し、結果を説明して。
```

wingetが見つからない場合は、Microsoft Storeで`アプリ インストーラー（App Installer）`を更新してから、ChatGPTデスクトップアプリを再起動します。

次に、Codexへ次のように指示します。

```text
wingetを使ってGit、Python、Node.js LTS、Pandocをインストールして。
git --version、python --version、py --version、node --version、npm --version、pandoc --versionを実行し、使えることを確認して。
wingetを使ってPython、Node.js LTS、Pandocをインストールして。
```

インストール後にソフトが見つからない場合は、ChatGPTデスクトップアプリを再起動し、もう一度確認します。
...
以後、この宿題のPython作業ではfinance-env\Scripts\python.exeを使って。
```
...
```text
wingetを使ってVisual Studio Codeをインストールして。Google Chromeが入っていなければ、Google Chromeもインストールして。
code --versionを実行し、Visual Studio Codeが使えることも確認して。
wingetを使ってVisual Studio Codeをインストールして。
```
...
> **🚨 動画での修正点**
>
> Visual Studio Codeのインストール後、Codexの権限を元に戻してください。

## 5. Visual Studio Codeを準備する

Visual Studio Codeを開き、Codexで使った`HW01`フォルダを開きます。

拡張機能画面（`Ctrl + Shift + X`）から、次をインストールします。
...
Visual Studio Codeで`chapter10_summary.md`を開き、Markdownの中身とプレビューを確認します。Markdown previewは`Ctrl + Shift + V`で開けます。

> [!CAUTION]
> **🚨 動画で触れなかった点：保存の印**
>
> ファイルを編集すると、タブの名前の右側の × が ● に変わります。変更がまだ保存されていない印です。`Ctrl + S` で保存すると ● が消えます。Codexやターミナルが読むのは保存済みのファイルなので、● が付いたまま作業を頼むと、保存前の古い内容が使われます。Codexに頼む前と、提出するファイルを作る前に、● が残っていないか確かめます。
...
Pythonはドキュメント\金融論\finance-env\Scripts\python.exeを使って。
